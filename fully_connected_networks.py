...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        db = torch.mm(dout.t(), torch.ones((N, 1), dtype=dx.dtype)).squeeze(1)
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    """
    # N = x.shape[0]
    # exp_matrix = exponential_func(x)
    # exp_sum = torch.sum(exp_matrix , axis = 1)
    # exp_sum = exp_sum.view(-1,1)
    # softmax_matrix = exp_matrix/exp_sum
    # y_hat = softmax_matrix[torch.arange(N), y]
    # CE_log = log_func(y_hat)
    # CE_loss = (-1/N)*torch.sum(CE_log).item()
    #
    # loss = torch.tensor(CE_loss,dtype=torch.float64)
    #
    # # Gradient computation
    #
    # dx = softmax_matrix.clone()
    ...
    y_hat = softmax_matrix[torch.arange(N), y]
    loss = -torch.mean(torch.log(y_hat), dtype=torch.float64)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dA1 , dW2 , db2 = Linear.backward(dx , cache2)
        dW2 =  dW2 + 2 * self.reg * self.params['W2']
        
        dA0 , dW1 , db1 = Linear_ReLU.backward(dA1 , cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i_layer in range(self.num_layers):
            w_i = torch.normal(0.0, weight_scale, dtype=dtype,
                               size=(dims[i_layer], dims[i_layer + 1])).to(device)
            b_i = torch.zeros(dims[i_layer + 1], dtype=dtype, device=device)
            W_key = 'W' + str(i_layer+1)
            b_key = 'b' + str(i_layer+1)
            self.params[W_key] = w_i
            self.params[b_key] = b_i
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for layer_num in range (self.num_layers) :
            W_key = 'W' + str(layer_num+1)
            b_key = 'b' + str(layer_num+1)
            if (layer_num==self.num_layers-1):
                scores , cache = Linear.forward(scores , self.params[W_key] ,
                                                 self.params[b_key])
                caches.append(cache)
            else :
                scores , cache = Linear_ReLU.forward(scores , self.params[W_key] ,
                                                      self.params[b_key])
                if self.use_dropout:
        ...
        for i_layer in reversed(range(self.num_layers)):
            if i_layer == self.num_layers - 1:
                ...
                dA, dw, db = Linear.backward(dA, cache)
                ...
                if self.use_dropout:
                    ...
                    dA = Dropout.backward(dA, cache_drop)
                else:
                    cache = caches[i_layer]
                dA, dw, db = Linear_ReLU.backward(dA, cache)
                dw = dw + 2 * self.reg * self.params[f'W{i_layer + 1}']
                grads[f'W{i_layer + 1}'] = dw
                grads[f'b{i_layer + 1}'] = db
        ###########################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    ##################################################################
    # Replace "pass" statement with your code
    variable1 = config['momentum']
    variable2 = config['learning_rate']
    v = variable1 * v - variable2 * dw
    next_w = w + v
    ###################################################################
    #                           END OF YOUR CODE                      #
    ###################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            # mask = torch.rand(x.shape).to(x.dtype).to(x.device)
            # mask = (mask < (1 - p))
            # mask = mask / (1 - p)
            # out = mask * x
